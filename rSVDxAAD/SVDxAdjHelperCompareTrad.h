...
// rSVDxAAD Interface to compare with Traditional Ajoint Formula and print all relevant error indicators (and sum to compare to SVDxAdjHelper results)
...
public:
    ...
    void run()
    {        
        BDCSVD<MatrixXd> m_svdA(m_A, Eigen::ComputeThinU | Eigen::ComputeThinV);
        m_SingValsTrad = m_svdA.singularValues();
        m_U0 = m_svdA.matrixU();
        m_V0 = m_U0.transpose();
...
private:
    ...
    void fAdjointTrad(const MatrixXd& A_bar, MatrixXd& A_bar_res)
    {
        ...
        for (size_t i = 0; i < m_N; i++)
        {
            ...
            {
                // # ARTICLE: formula (5) F
                if (fabs(m_SingValsTrad[i]-m_SingValsTrad[j]) > 1e-6)
                    F(i,j)=(myStep(m_SingValsTrad[i])-myStep(m_SingValsTrad[j])) / (m_SingValsTrad[i]-m_SingValsTrad[j]);
                else
                    F(i,j)=myStepDeriv(m_SingValsTrad[i]);
            }
        } 

        MatrixXd B = (m_V0 * A_bar * m_U0).cwiseProduct(F);
        A_bar_res = m_U0 * B * m_V0;
    }
    ...
    void compareDerivatives()
    {
        ...
        for (size_t i = 0; i < m_N; i++)
            for (size_t j = 0; j < i+1; j++)
            {
                ...
                E(j,i) += 1;
                fAdjoint(E, A_bar);
                fAdjointTrad(E, A_barTrad);

                for (size_t k = 0; k < m_N; k++)
                    for (size_t l = 0; l < k+1; l++)
                    {
                        sumAbsA_bar += fabs(A_bar(k,l));
                        sumAbsA_barTrad += fabs(A_barTrad(k,l));

                        if (fabs(A_barTrad(k,l)) != 0)
                        {
                            errorRel += fabs(A_barTrad(k,l) - A_bar(k,l))/fabs(A_barTrad(k,l));
                            nPoints++;
                        }
                        else
                            nPoints0s++;
                        
                        MSE += (A_barTrad(k,l) - A_bar(k,l))*(A_barTrad(k,l) - A_bar(k,l));
                    } 
            }  
        errorRel /= nPoints;
        MSE /= nPoints;
        ...
        std::cout << "(Adjoint) sumAbsA_barTrad: " <<  sumAbsA_barTrad << std::endl; 
    }
};
